...
# WLS (CHARLES, USE THIS ONE FOR GETTING PERFORMANCE OF WLS)
...
y_hat.weight.train <- predict(fit_price_train_trans_w2, newdata = df_train_linear_out_step_vif)
MAE_train <- mae(df_train_linear_out_step_vif$price, y_hat.weight.train)
SSE_train <- sum((df_train_linear_out_step_vif$price - y_hat.weight.train )^2)
R_squared_train <- R2(df_train_linear_out_step_vif$price, y_hat.weight.train)
RMSE_train <- rmse(df_train_linear_out_step_vif$price, y_hat.weight.train)
MAE_train
SSE_train
R_squared_train
RMSE_train
...
y_hat.weight.test <- predict(fit_price_train_trans_w2, newdata = df_test_linear_step_vif)
MAE_test <- mae(df_test_linear_step_vif$price, y_hat.weight.test)
SSE_test <- sum((df_test_linear_step_vif$price - y_hat.weight.test)^2)
R_squared_test <- R2(df_test_linear_step_vif$price, y_hat.weight.test)
RMSE_test <- rmse(df_test_linear_step_vif$price, y_hat.weight.test)
MAE_test
SSE_test
R_squared_test
RMSE_test
...
#Performance in the training dataset
SSE_train <- fit_logistic_step$deviance
SST_train <- fit_logistic_step$null.deviance
R_squared_train <- 1 - SSE_train / SST_train
RMSE_train <- sqrt(fit_logistic_step$deviance/summary_fit_logistic$df.residual)
SSE_train
R_squared_train
RMSE_train
# MAE_train <- mae(true_labels, predictions)
# SSE_train <- sum((true_labels - predictions)^2)
# R_squared_train <- R2(true_labels, predictions)
# RMSE_train <- rmse(true_labels, predictions)
# MAE_train
# SSE_train
# R_squared_train
# RMSE_train
...
#Performance in the test dataset
fit_logistic_test_null <- glm(price ~ 1, family = binomial, data = df_hs_test_logi)


SSE_test <- sum((predictions - df_hs_test_logi$price)^2)
MAE_test <- mean(abs(predictions - df_hs_test_logi$price))
R_squared_test <- 1 - SSE_test / fit_logistic_test_null$null.deviance
R_squared_test
RMSE_test <- sqrt(mean((predictions - df_hs_test_logi$price)^2))
SSE_test 
MAE_test 
R_squared_test
RMSE_test

pred.out.link <- predict(fit_logistic_step, newdata = df_hs_test_logi, type = "link")
mod.out.null <- glm(price ~ 1, family = binomial, data = df_hs_test_logi)
pR2.out <- 1 - sum(log(1 + exp(pred.out.link))) / logLik(mod.out.null)[1]
pR2.out

# MAE_test <- mae(true_labels, predictions)
# SSE_test <- sum((true_labels - predictions)^2)
# R_squared_test <- R2(true_labels, predictions)
# RMSE_test <- rmse(true_labels, predictions)
# MAE_test
# SSE_test 
# R_squared_test
# RMSE_test
