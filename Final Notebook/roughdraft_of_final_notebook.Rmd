...
boxplot(train_df_non_linear$price ~ factor(train_df_non_linear$waterfront_1, levels = c(0, 1)),
        main = "Price vs. Waterfront", xlab = "Waterfront", ylab = "Price", col = "purple",
        xaxt = "n")  # Setting xaxt to "n" to customize x-axis labels
axis(side = 1, at = c(1, 2), labels = c("No", "Yes"))  # Customizing x-axis labels
...
barplot(table(train_df_non_linear$waterfront_1), main = "Distribution of Waterfront",
        xlab = "Waterfront", ylab = "Frequency",
        names.arg = c("No", "Yes"), col = "purple")  # Customizing x-axis labels and color
...
# Refactor the Season variable to reflect temporal order
train_df_non_linear$Season <- with(train_df_non_linear,
                                   ifelse(season_Spring == 1, "Spring",
                                          ifelse(season_Summer == 1, "Summer",
                                                 ifelse(season_Fall == 1, "Fall", "Winter"))))

# Set the factor levels for Season
train_df_non_linear$Season <- factor(train_df_non_linear$Season, levels = c("Spring", "Summer", "Fall", "Winter"))

# Seasonal Trends in Average House Prices
seasonal_trends <- aggregate(price ~ Season, data = train_df_non_linear, FUN = mean)

# Find the global maximum
global_max_seasonal <- seasonal_trends[which.max(seasonal_trends$price), ]

# Define colors for seasons
season_colors <- c("Spring" = "green", "Summer" = "yellow", "Fall" = "orange", "Winter" = "blue")

ggplot(seasonal_trends, aes(x = Season, y = price, fill = Season)) +
  geom_bar(stat = "identity") +
  geom_text(data = global_max_seasonal, aes(x = Season, y = price, label = paste("Global Max:", round(price, 2))), vjust = -0.5) +
...
ggplot(seasonal_counts_df, aes(x = Season, y = Count, fill = Season)) +
  geom_bar(stat = "identity") +
  geom_text(data = global_max_count_seasonal, aes(x = Season, y = Count, label = paste("Global Max:", Count)), vjust = -0.5) +
  scale_fill_manual(values = season_colors) +
...
# Subsetting the train and test dataframes with the used features
tryCatch({
...
# step_model_both <- ols_step_both_p(linear_model_initial, pent=0.05, prem=0.05)
# features_both <- names(coef(step_model_both))

# # Stepwise Regression: Backward
# step_model_backward <- ols_step_backward_p(linear_model_initial, prem = 0.05)
# features_backward <- names(coef(step_model_backward))
#
# # Stepwise Regression: Forward
# step_model_forward <- ols_step_forward_p(linear_model_initial, penter = 0.05)
# features_forward <- names(coef(step_model_forward))

view_model_results(df_results)
```

```{r add_features}
# features_both <- extract_and_display_features(step_model_both$model, train_df_linear, "price")
