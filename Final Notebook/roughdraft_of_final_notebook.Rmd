...
required_packages <- c(
  ...
  "GGally", "ggplot2", "ggpubr", "glmnet", "lmtest", "MASS", "ModelMetrics",
  "nortest", "olsrr", "onewaytests", "readr", "here", "stringr", "knitr", "reshape2", "leaflet",
  "RColorBrewer", "scales", "purrr", "DT", "jsonlite", "magrittr", "rpart"
)
...
train_df_non_linear <- train_df_non_linear[, !colnames(train_df_non_linear) %in% c("view_category", "condition_category", "grade_category", "grade_category_numeric", "z_score", "lat", "long", 'sqft_above', 'month_sold', 'week_of_year', 'condition_3', "grade_Below_Average")]
train_df_linear <- train_df_linear[, !colnames(train_df_linear) %in% c("view_category", "condition_category", "grade_category", "grade_category_numeric", "z_score", "lat", "long", 'sqft_above', 'month_sold', 'week_of_year', 'condition_3', "grade_Below_Average")]
test_df_linear <- test_df_linear[, !colnames(test_df_linear) %in% c("view_category", "condition_category", "grade_category", "grade_category_numeric", "z_score", "lat", "long", 'sqft_above', 'month_sold', 'week_of_year', 'condition_3', "grade_Below_Average")]
test_df_non_linear <- test_df_non_linear[, !colnames(test_df_non_linear) %in% c("view_category", "condition_category", "grade_category", "grade_category_numeric", "z_score", "lat", "long", 'sqft_above', 'month_sold', 'week_of_year', 'condition_3', "grade_Below_Average")]
...
```{r residuals_vs_fitted_plot, echo = FALSE}
...
```{r scale_location_plot, echo = FALSE}
...
```{r residuals_vs_leverage_plot, echo = FALSE}
...
```{r cooks_distance_plot, echo = FALSE}
...
### Diagnostics for Homoscedasticity, Normality of Errors and Constancy of Error Variances
...
```{r boxcox, echo = FALSE, include = TRUE}
par(mfrow=c(1,1))
...
transformed_model <- lm(transformed_price ~ ., data = train_df_linear)
...
# Back-transform the predicted prices to original scale
if (optimal_lambda != 0) {
  predicted_train <- (predicted_train_transformed * optimal_lambda + 1)^(1 / optimal_lambda)
  predicted_test <- (predicted_test_transformed * optimal_lambda + 1)^(1 / optimal_lambda)
} else {
  predicted_train <- exp(predicted_train_transformed)
  predicted_test <- exp(predicted_test_transformed)
}

# Calculate SSE for the training and test data
sse_train <- sum((train_df_linear$price - predicted_train)^2)
sse_test <- sum((test_df_linear$price - predicted_test)^2)

# Calculate SST for test data
sst_test <- sum((test_df_linear$price - mean(test_df_linear$price))^2)

# Calculate R-squared for training and test data
r_squared_train <- summary(transformed_model)$r.squared
r_squared_test <- 1 - (sse_test / sst_test)

# Calculate RMSE for training and test data
rmse_train <- sqrt(sse_train / nrow(train_df_linear))
rmse_test <- sqrt(sse_test / nrow(test_df_linear))

# Calculate MAE for training and test data
mae_train <- mean(abs(train_df_linear$price - predicted_train))
mae_test <- mean(abs(test_df_linear$price - predicted_test))
...
```{r transformed_model_plots, echo = FALSE, include = TRUE}
...
```{r residuals_vs_leverage_plot_v2, echo=FALSE}
...
```{r residuals_vs_fitted_plot_v2, echo=FALSE}
...
```{r cooks_distance_calculation, echo=FALSE}
...
train_df_linear_copy <- train_df_linear
test_df_linear_copy <- test_df_linear
...
train_df_linear_copy <- remove_high_vif_columns(train_df_linear_copy, vif_threshold)
test_df_linear_copy <- remove_high_vif_columns(test_df_linear_copy, vif_threshold)

# Fit a linear regression model to the updated training data
linear_model_initial_vif <- lm(price ~ ., data = train_df_linear_copy)

# Evaluate the updated linear model on the copied test data
df_results <- evaluate_model("OLS_linear_VIF_10", linear_model_initial_vif, train_df_linear_copy, test_df_linear_copy, target_var = 'price', df_results)
...
abs_residuals_model <- lm(abs_residuals ~ ., data = train_df_linear[, -1])
...
weighted_regression_model <- lm(log(price) ~ ., weights = weights, data = train_df_linear)
...
# Calculate predictions for training and test data using the weighted model
predicted_train_transformed_wls <- predict(weighted_regression_model, newdata = train_df_linear)
predicted_test_transformed_wls <- predict(weighted_regression_model, newdata = test_df_linear)

# Back-transform the predicted prices to the original scale
predicted_train_wls <- exp(predicted_train_transformed_wls)
predicted_test_wls <- exp(predicted_test_transformed_wls)  # Back-transform for test data

# Calculate SSE for the training data
sse_train_wls <- sum((train_df_linear$price - predicted_train_wls)^2)

# Calculate SSE for the test data
sse_test_wls <- sum((test_df_linear$price - predicted_test_wls)^2)

# Calculate R-squared for training data
r_squared_train_wls <- 1 - sse_train_wls / sum((train_df_linear$price - mean(train_df_linear$price))^2)

# Calculate R-squared for test data
r_squared_test_wls <- 1 - sse_test_wls / sum((test_df_linear$price - mean(test_df_linear$price))^2)

# Create a dataframe to store the results
df_results <- rbind(df_results, data.frame(
  Model = "WLS_linear",
  SSE_train = sse_train_wls,
  SSE_test = sse_test_wls,
  R_squared_train = r_squared_train_wls,
  R_squared_test = r_squared_test_wls,
  RMSE_train = sqrt(sse_train_wls / nrow(train_df_linear)),
  RMSE_test = sqrt(sse_test_wls / nrow(test_df_linear)),
  MAE_train = mean(abs(train_df_linear$price - predicted_train_wls)),
  MAE_test = mean(abs(test_df_linear$price - predicted_test_wls))
))
...
```{r refit_all_OLS_models, echo = FALSE}
# Fit a linear regression model to the training data
