...
required_packages <- c(
  ...
  "RColorBrewer", "scales", "purrr", "DT", "jsonlite", "magrittr", "rpart", "keras",
  "reticulate"
)
...
**Points of Concern**: Specific points, notably labeled 830 and 19140, stray from the general cluster, hinting at potential outliers or influential observations.
...
**Specific Note**: The points at the upper end, including 4320 and 19140, are especially distant from the line, suggesting they could be outliers with a substantial effect on our model's performance.
...
- **Test Statistic (BP)**: `r bp_test_results$statistic`
- **Degrees of Freedom (df)**: `r bp_test_results$parameter`
- **P-value**: `r bp_test_results$p.value`

Given that our p-value is `r ifelse(bp_test_results$p.value < 0.05, "less", "greater")` than 0.05, we `r ifelse(bp_test_results$p.value < 0.05, "reject", "fail to reject")` the null hypothesis, indicating that `r ifelse(bp_test_results$p.value < 0.05, "heteroscedasticity is likely present", "the assumption of homoscedasticity holds")` in our residuals.
...
- **Observation**: Inspect the plot for patterns, curvature, or outliers in residuals.
...
- **Observation**: Cook's distance quantifies the influence of each observation. High values indicate strong influence.
...
- **Conclusion**: The `outliers` dataframe contains data points with high influence on the model.
...
train_and_evaluate_keras_nn <- function(train_data, test_data, original_train_price, original_test_price) {
  ...
  best_model <- NULL
  lowest_val_mae <- Inf
  batch_sizes <- c(32, 64, 128, 256)  # Different batch sizes to experiment with
  epochs_list <- c(50, 100, 150)      # Different numbers of epochs to experiment with
...
# Check if 'price' column exists in the datasets
if (!"price" %in% colnames(train_df_reg)) {
  stop("Column 'price' not found in train_df_reg")
}
if (!"price" %in% colnames(test_df_reg)) {
  stop("Column 'price' not found in test_df_reg")
}
...
p.rpart_train <- predict(m.rpart, newdata = train_df_reg)
p.rpart_test <- predict(m.rpart, newdata = test_df_reg)

# MAE, SSE, R2, RMSE Calculation Functions
...
SSE_train <- SSE(train_df_reg$price, p.rpart_train)
...
SSE_test <- SSE(test_df_reg$price, p.rpart_test)
